...
void move_ex6()
{
  ...
  std::copy_if(
    std::make_move_iterator(std::begin(src4)),
    std::make_move_iterator(std::end(src4)),
    std::back_inserter(dst4),
    [](const auto &p){ return *p == 3; }
  );
  ASSERT(dst4.size() == 1);
  ASSERT(*dst4[0] == 3);
  ASSERT(src4.size() == 3);
  ...
  /*
   * Now try to do the same with std::accumulate
   * Figure out why it make a difference
   */
  std::vector<std::string> strings = {"Hello ", "from ", "std::accumulate"};
  std::string concat = std::accumulate(
    std::make_move_iterator(strings.begin()),
    std::make_move_iterator(strings.end()),
    std::string());
  ASSERT(concat == "Hello from std::accumulate");
}
